...
export function AccessControl({ stats }: AccessControlProps) {
  const fullAccess = stats?.byAccess['Full Access'] || 0;
  const restricted = stats?.byAccess['Restricted'] || 0;
  const total = fullAccess + restricted || 1;
  const fullPct = (fullAccess / total) * 100;
  const restrictedPct = (restricted / total) * 100;

  const win11 = stats?.byOS['Windows 11 Pro'] || 0;
  const win10 = stats?.byOS['Windows 10 Pro'] || 0;
  const unknown = stats?.byOS['Unknown'] || 0;
      ...
      <CardContent>
        <div className="flex gap-3 mt-1">
          ...
          <div className="flex-1 bg-slate-50 rounded-lg p-3 text-center">
            ...
            <div className="text-[11px] text-slate-500">Restricted</div>
          </div>
        </div>
        <div className="mt-3">
        ...
        <div className="mt-3">
          <div className="text-[11px] text-slate-400 mb-2">OS breakdown</div>
